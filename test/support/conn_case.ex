defmodule PhoenixBoilerplate.ConnCase do
  @moduledoc """
  ...
  using do
    quote do
      ...
      @endpoint PhoenixBoilerplate.Endpoint
      ...
      def guardian_login(%PhoenixBoilerplate.User{} = user), do: guardian_login(conn(), user, :token, [])
      def guardian_login(%PhoenixBoilerplate.User{} = user, token), do: guardian_login(conn(), user, token, [])
      def guardian_login(%PhoenixBoilerplate.User{} = user, token, opts), do: guardian_login(conn(), user, token, opts)
      ...
      def guardian_login(%Plug.Conn{} = conn, user, token, opts) do
        conn
          |> bypass_through(PhoenixBoilerplate.Router, [:browser])
          |> get("/")
